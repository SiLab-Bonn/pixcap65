...
def advanced_analysis_delegate(file: tb.File, data_group: tb.Group, result_group: tb.Group, use_kafe2=True, is_cv=False,
    ...
    for ii, jj in np.indices(current_hist.shape[:2]):
        if np.isfinite(current_hist[ii, jj, 0]):
            if use_kafe2:
                ...
                cost = LeastSquares(x=scan_parameters['frequency'], y=current_hist[ii, jj],
                                    yerror=errors,
                                    model=full_capacitance_model if full_model else simple_capacitance_model)
                if full_model:
                    m = Minuit(cost, c=1e-6, r=1e6, i=0)
                else:
                    m = Minuit(cost, c=1e-6, i=0)
                m.fixto('u0', 1)
                m.migrad()
                m.hesse()
